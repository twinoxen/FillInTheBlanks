import * as myNumFuncs from "../../../src/number";
...
describe("numbers functions", () => {
  ...
  it("can find the square root of any number", () => {
    // hint: use the ** operator... more on operators later
    expect(myNumFuncs.squareRoot(8)).toBe(64);
  });

  it("can calculate a tip for good service (20% of total bill)", () => {
  ...
  it("can parse whole numbers from a string representing a number", () => {
    // hint: use parseInt()
    expect(myNumFuncs.stringParserInt("42")).toStrictEqual(42); // * notice parseInt converts a string to a number
  });

  it("can parse numbers with decimals (a.k.a. floats) from a string representing a number with decimals", () => {
    // hint: use parseFloat()
    expect(myNumFuncs.stringParserFloat("42.333")).toStrictEqual(42.333); // * notice parseFloat converts a string to a number
  });

  it("can use the Number primitive to parse strings into numbers", () => {
  ...
  it("can roll a 6 sided die", () => {
    ...
    expect(myNumFuncs.roll6D()).toBeGreaterThanOrEqual(1);
    expect(myNumFuncs.roll6D()).toBeLessThan(7);
  });
});
